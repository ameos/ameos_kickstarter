		...
		foreach ($fields as $fieldname => $filetype) {
			...
			switch ($filetype) {
				case 'string':
					...
					$tcaFieldType = fgets($fileResource, 1024);
					$tcaFieldType = trim($tcaFieldType);
					switch ($tcaFieldType) {
						case 'input':
						...
						default:
							$sqlFields  .= PHPTAB .$fieldname . ' varchar(255) DEFAULT \'\' NOT NULL,' . LF;
							$this->warning .= 'TCA type "'.$tcaFieldType.'" not found for field "'.$fieldname.'", please check manualy TCA file and ext_tables.sql' . LF;
							break;
					}
					fclose($fileResource);	
					break;
				...
				default:
					...
					if(substr( $filetype, 0, 36 ) === 'Tx_Extbase_Persistence_ObjectStorage' || substr( $filetype, 0, 44 ) === '\\TYPO3\\CMS\\Extbase\\Persistence\\ObjectStorage')
					{
					...
					{
						$tcaPhpCode .= PHPTAB . PHPTAB . PHPTAB . PHPTAB . '\'foreign_table\' => \''.'tx_'.str_replace('_', '', $this->extensionKey).'_domain_model_'.strtolower($filetype).'\',' . LF;
						
						$sqlFields  .= PHPTAB .$fieldname . ' int(11) DEFAULT \'0\' NOT NULL,' . LF;
						
						$this->warning .= 'Type "'. $filetype .'" assumed to be model of extension "'. $this->extensionKey .'"' . LF;
					}
					break;
			}
